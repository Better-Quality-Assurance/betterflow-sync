...
class OfflineQueueProtocol(Protocol):
    ...
    def dequeue(self, batch_size: int = 100, max_retries: int = 5) -> list: ...
    ...
    # Restart-safe per-event counted-time dedup. SyncEngine relies on these to
    # avoid re-counting an event's active seconds across restarts/reconciles.
    # (SyncEngine still guards the calls defensively so plain Mock queues in
    # tests stay tolerant, but the contract belongs on the protocol.)
