...
SUPPRESS_ENV_VAR = "BETTERFLOW_SUPPRESS_NOTIFICATIONS"


def _notifications_suppressed() -> bool:
    """True when this process must not post a REAL notification.

    The only guard used to be an autouse PYTEST FIXTURE
    (tests/conftest.py::_block_real_notifications), which binds nothing outside
    pytest -- so any ad-hoc script driving the real code path notified the actual
    user. On 2026-09-04 one did: a review agent's probe drove
    `_rosetta_required=True` without mocking `_notify_rosetta_required_once` and
    the repo owner got a live "Rosetta 2 is required" notification generated by a
    test. Agents in this repo drive `_start_locked` in probes constantly.

    Opt-IN, and deliberately strict about what counts as opting in. A stray empty
    or "0" value must not silently disable every notification the product sends:
    that failure is worse than the one this closes, because it is invisible --
    the app keeps running and the user simply never hears from it again.
    """
    return os.environ.get(SUPPRESS_ENV_VAR, "").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def send_notification(
    ...
    if _notifications_suppressed():
        # UNSUPPORTED, not DELIVERED: there is deliberately no channel here, and
        # only an OBSERVED delivery may claim DELIVERED (module docstring).
        # Returning DELIVERED would be exactly the lie NotificationOutcome
        # exists to prevent, one layer up.
        logger.debug("Notification suppressed by %s: %r", SUPPRESS_ENV_VAR, title)
